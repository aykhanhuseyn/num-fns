...
interface AmountParts {
  isNegative: boolean
  /** Whole major units, exact at any magnitude and read by `numberToWords`' `bigint` path whatever the input type. */
  major: bigint
 ...
 * to the currency's `decimals` with `arithmetic/round` (via `splitFixed`) —
