...
describe('round', () => {
  ...
  it('keeps the sign of a negative value that rounds away to zero', () => {
    ...
    expect(Object.is(round(-0.5, 0, 'halfDown'), -0)).toBe(true)
    expect(Object.is(round(-0.5, 0, 'halfEven'), -0)).toBe(true)
    expect(Object.is(round(-0.5, 0, 'ceil'), -0)).toBe(true)
    expect(Object.is(round(-7, -2), -0)).toBe(true)
