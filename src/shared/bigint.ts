 ...
 *   {@link scaleBigInt} divides by a scale factor with exact half-up
 *   rounding, standing in for `(value / threshold).toFixed(decimals)`.
 *
...
/**
 * Divides `value` by `factor` and renders the quotient with exactly
 * `decimals` fractional digits, rounded half up on the exact remainder —
 * the `bigint` stand-in for `(value / factor).toFixed(decimals)` in
 * `toShortNotation` and `toByteSize`. `value` must be non-negative, `factor`
 * positive and `decimals` a non-negative integer (the callers validate).
 *
 * `scaleBigInt(BigInt(1536), BigInt(1024), 2)` is `"1.50"`.
 */
export function scaleBigInt(value: bigint, factor: bigint, decimals: number): string {
  const shifted = value * pow10(decimals)
  // (2·shifted + factor) / (2·factor) is ⌊shifted / factor + ½⌋ in integers.
  const rounded = (shifted * TWO + factor) / (factor * TWO)
